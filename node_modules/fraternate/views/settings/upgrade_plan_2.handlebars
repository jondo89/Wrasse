...
{{else}}
	...
	<hr>
	<div class="btn-group"  >
		<a href="/users/{{user.username}}/settings/billing/upgrade" class="btn btn-outline-secondary btn-lg ">
			<input type="radio" name="options" id="option1" autocomplete="off" style="position: relative;" > <strong>$7 / month </strong>
		</a>
		<a href="/users/{{user.username}}/settings/billing/upgrade_plan_2" class="btn btn-outline-secondary btn-lg active" >
			<input href="" type="radio" name="options" id="option2"  autocomplete="off" style="position: relative;"  checked>  <strong>$84 / year </strong></a>
		</a>
	</div>
		...
		<div class="awesome">
			<div class="panel panel-default" style="margin-bottom: 1px;border-color: #5cb85c; "  >
				<div class="panel-body" >
					Charge today
					<h1 id="pricingNow">$84.00</h1>
				</div>
				<div class="panel-footer">Your next charge will be on <span id="dates"></span>.</div>
			</div>
		</div>
		<p></p>   <p>
		By clicking on "Upgrade your plan" below, you are agreeing to the <a href="/terms" target="_blank">Terms of Service</a> and the <a href="/privacy" target="_blank">Privacy Policy</a>.
	</p>
	<div class="btn-group" role="group" aria-label="...">
		<a type="button" class="btn btn-success disabled" id="submits" type="submit">Upgrade your plan</a>
		<a type="button" class="btn btn-outline-secondary" href="/users/{{user.username}}/settings/billing/">Cancel</a>
	</div>
...
function formPost(payload){
	...
	$.ajax({
		type: "POST",
		url: '/users/'+username+'/subscription_plan_2',
		data: datastring,
		dataType: "json",
